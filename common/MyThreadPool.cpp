...
int MyThreadPool::open(size_t thread_num, size_t stack_size ,FILE * (* binFiles)[THREAD_NUM])
{
	int ret = -1;
	size_t i;
	m_binfiles = binFiles;
...
int MyThreadPool::svc()
{
	Worker * worker;
	int queueLen;
	size_t thread_id = (size_t)(current_thread_num++);
...
int MyThreadPool::wait_worker_done(const std::vector<Worker*>& workers){
    ...
    cerr << "debug info :: " << len << " jobs is running " << endl;
	while(len>0){
