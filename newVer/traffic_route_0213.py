...
class TrafficNet:
    ...
    def __dump_all(self):
        #N转交通的条数{0:23,1:34}
        transit_count = {}
        #N转交通的详细信息{0:['key1','key2','key3'],1:['key1','key2']}
        transit_info = {}
	for s in self:
...
def find_all():
    ...
    for city in city_list:
        ...
        #统计该城市N转交通路线的总的条数{0:234,1345}
        transit_count_res = {}
        #统计该城市N转交通路线的所有key{0:[key1,key2]}
        transit_info_res = {}
        #统计交通站点的个数
        station_num = 0
        for station in traffic_net:
            ...
            for key,val in transit_info.items():
                if key in transit_info_res:
                   ...
                   transit_info_res[key] = val
        #N转交通路线信息转换为json字符串
        for key,val in transit_info_res.items():
            transit_info_res[key] = val[:10]
        ...
        for key,val in transit_count_res.items():
            print_res.append("%d 转交通路线条数%d 及其所占的百分比：%f"%(key,val,100*(float(val)/routes_num)))
        for key,val in transit_info_res.items():
            if key > 3:
               print_res.append("%d 转交通路线的case %s"%(key,print_N_path(city,str(key))))
        print_res.append('\n')
    out_file = open('all_log.txt','w')
...
def print_N_path(city,num):
    ...
    for index in range(key_len):
        ...
        path = select_path.parse_path(path)
        path_res += path + '\n'
    return path_res
	...
	for stationNode in station_list:
     ...
     for k in dic:
	   ...
	   #r1.set(k.strip(),str(dic[k]))
            ssdb.request('set',[k.strip(),str[dic[k]]])
