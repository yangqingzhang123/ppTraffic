...
class TrafficNet:
    ...
    def __dump_all(self):
        #N转交通条数{0:23,1:34}
        transit_count = {}
        #N转交通的详细信息{0:[key1,key2,key3]}
        transit_info = {}
	for s in self:
	    path=s.path_dump(self)
	    if path:
                str_key=path["start_map"]+"###"+path["dest_map"]+"@0@0"
                #统计以某个站点为起点的N转交通的条数
                transit_key = path["transit_num"]
                if transit_key in transit_count:
                   transit_count[transit_key] += 1
                   transit_info[transit_key].append(str_key) 
                else:
                   transit_count[transit_key] = 1
                   transit_info[transit_key] = [str_key]
	        r0.set(str_key,path)
                #ssdb.request('set',[str_key,path])
        return transit_count,transit_info
    ...
    def find_route(self,start):
        ...
        #N转交通路线的条数
        transit_count,transit_info = self.__dump_all()
        return transit_count,transit_info
...
def find_all():
    ...
    city_list=[]
    print_res = []
    with open('../data/city01.list', 'r') as f:
        for line in f.readlines():
    ...
    for city in city_list:
        ...
        for station in traffic_net:
            station_num += 1
            transit_count,transit_info = traffic_net.find_route(station)
            #统计所有交通路线N转交通的条数
            for key,val in transit_count.items():
                if key in transit_count_res:
                   transit_count_res[key] += val
                else:
                   transit_count_res[key] = val
            for key,val in transit_info.items():
                if key in transit_info_res:
                   transit_info_res[key] += val
                else:
                   transit_info_res[key] = val  
        json_transit_info_res = json.dumps(transit_info_res)
        ssdb.request('set',[city,json_transit_info_res])
        routes_num = station_num * (station_num - 1) 
        print_res.append(city)
        for key,val in transit_count_res.items():
            print_res.append("%d 转交通路线条数%d 及其所占的比例：%f"%(key,val,100*(float(val)/routes_num)))
        for key,val in transit_count_res.items():
            if key > 3:
               print_res.append("%d 转交通路线的case %s"%(key,print_N_path(city,str(key))))
        print_res.append('\n')
        generate_poi_dic(city)
        find_all_path(city)
    out_file = open('log01.txt','w')
    for item in print_res:
        out_file.write(item+'\n')
    out_file.close()
    
def print_N_path(city,num):
    pathKey = str(ssdb.request('get',[city]))
    pathKey = pathKey[8:]
    pathKey_dic = json.loads(pathKey)
    key_list = pathKey_dic[num]
    key_len = 3
    if len(key_list) < 3:
        key_len = len(key_list)
    path = ""
    for index in range(key_len):
        key = pathKey_dic[num][index]
        path += str(ssdb.request('get',[key]))[8:]
        path += '\n'
    return path
...
def find_all_path(city_id):
    ...
    pathes_dic = {}
    file_num = 0
    with open("%s.query"%city_id,'r') as f:
	for line in f.readlines():
            if line.strip() == "######":
               pathes_dic = {}
               if len(path_list) >= PATHNUM:
               ...
               paths = find_path(start,start_type.strip(),dest,dest_type.strip(),city_id,pathes_dic)
               for path in paths:
...
#找两个poi点之间的最优路径
def find_path(start,start_type,dest,dest_type,city_id,pathes_dic):    
     ...
     if dis <= 1000:
        ...
        if pathStr == "d None None" or pathStr == "not_found None None":
           ...
           pathStr = pathStr[8:]
        if start_type == "0":
           strKey = start.split('#')[-1]+"###"+dest.split('#')[-1]+"@0@0"
           pathes_dic[strKey] = pathStr
        return [pathStr]
     ...
     if start_type == "1" and dest_type == "1":
        ...
        res_path = find_path_sub_1_1(start,dest,start_nearest_station,city_id,pathes_dic)
     #起始poi是非交通站点，得到该poi点附近的十个站点
        ...
        start_nearest_station = heapq.nsmallest(10,start_nearest_station,key=lambda x:x[1])
        start_nearest_station = pruning_nearest_station(start_nearest_station)
        res_path = find_path_sub_1_0(start,start_nearest_station,dest,city_id,pathes_dic)
     #终止poi是非交通站点，得到该poi点附近的十个站点
        ...
        res_path = find_path_sub_0_1(start,dest,dest_nearest_station,city_id,pathes_dic)
     elif start_type == "0" and dest_type == "0":
        res_path = find_path_sub_0_0(start,dest,city_id,pathes_dic)
...
def find_path_sub_1_1(start,dest,start_nearest_station,city_id,pathes_dic):
     start_poi_map = start.split('#')[-1]
     dest_poi_map = dest.split('#')[-1]
     start_path_str_res = ""
     dest_path_str_res = ""
        ...
        if start_dis <= 3000:
           ...
           if start_path is None:
              ...
              if start_path is None:
                 start_path = line_walk_path(start,start_station[0],city_id,start_dis)
        dest_map = dest.split('#')[-1]
        strKey = start_map.strip()+"###"+dest_poi_map.strip()+"@0@0"
        pathStr = pathes_dic[strKey]             
        start_path_dic = eval(start_path)
        path_dic = eval(pathStr)
        tmp_score = int(path_dic["time"])+int(start_path_dic["time"]) 
...
def combine_path(start_path,dest_path,start_type,dest_type,pathStr):
    ...
    if start_type == "1" and start_path_dic["dist"] > 0.0:
        ...
        path_dic["score"] = int(start_path_dic["time"])+int(path_dic["time"])
